...
pkg_banner "GuestKit host dependencies" "qemu-img · nbd (pure Rust — not libguestfs)"
SUDO=""
[[ "$(id -u)" -ne 0 ]] && command -v sudo &>/dev/null && SUDO=sudo
pkg_info "Installing qemu-img, nbd, lvm2, parted…"
# Delegate to existing logic inline (same as before)
if command -v apt-get &>/dev/null; then
  $SUDO apt-get update -qq
  $SUDO apt-get install -y -qq qemu-utils nbd-client lvm2 parted 2>&1 | tail -5 || true
