...
pub fn infer_root_cause(
    ...
    let (primary, confidence) = if let Some(check) = boot.checks.iter().find(|c| c.id == "BOOT-001" && !c.passed)
    {
        ...
        chain.push(CauseStep {
            step: 2,
            description: "Kernel cannot mount root → initramfs drops to emergency shell".to_string(),
            check_id: None,
        });
        (
